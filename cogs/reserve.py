...
class Reserve(commands.Cog):
    ...
    def __init__(self, bot):
        ...
        if TransferData().download_file(r'/res.pkl',TEMP_PATH + 'res.pkl'):
            ...
            # Noneを除外しリストに
            self.res_b1 = list(filter(None, a))
            self.res_b2 = list(filter(None, b))
            self.res_b3 = list(filter(None, c))
            self.res_b4 = list(filter(None, d))
            self.res_b5 = list(filter(None, e))
        else:
            self.res_b1 = []
            self.res_b2 = []
            self.res_b3 = []
    ...
    def cog_unload(self):
        self.res = zip_longest(self.res_b1, self.res_b2, self.res_b3,
        self.res_b4, self.res_b5)
    ...
    def overlap_check(self, user):  # 重複防止
        list = self.res_b1+self.res_b2+self.res_b3+self.res_b4+self.res_b5
        for i in list:
            if user in i:
                return True
        return False
    ...
    async def res1(self, ctx, note=''):
        if self.overlap_check(ctx.author.display_name):
            ...
            return
        self.res_b1.append(ctx.author.display_name
    ...
    async def res2(self, ctx, note=''):
        if self.overlap_check(ctx.author.display_name):
            await ctx.send('予約はひとり1つまでです。')
            return
        self.res_b2.append(ctx.author.display_name
        if not note else ctx.author.display_name + f': {note}')
    ...
    async def res3(self, ctx, note=''):
        if self.overlap_check(ctx.author.display_name):
            await ctx.send('予約はひとり1つまでです。')
            return
        self.res_b3.append(ctx.author.display_name
        if not note else ctx.author.display_name + f': {note}')
    ...
    async def res4(self, ctx, note=''):
        if self.overlap_check(ctx.author.display_name):
            await ctx.send('予約はひとり1つまでです。')
            return
        self.res_b4.append(ctx.author.display_name
        if not note else ctx.author.display_name + f': {note}')
    ...
    async def res5(self, ctx, note=''):
        if self.overlap_check(ctx.author.display_name):
            await ctx.send('予約はひとり1つまでです。')
            return
        self.res_b5.append(ctx.author.display_name
        if not note else ctx.author.display_name + f': {note}')
    ...
    async def clear(self, ctx):  #予約を削除
        ...
        /凸完了 /完了 /クリア でも反応します。
        """
        for i, str in enumerate(self.res_b1):
            if ctx.author.display_name in str:
                del self.res_b1[i]
        for i, str in enumerate(self.res_b2):
            if ctx.author.display_name in str:
                del self.res_b2[i]
        for i, str in enumerate(self.res_b3):
            if ctx.author.display_name in str:
                del self.res_b3[i]
        for i, str in enumerate(self.res_b4):
            if ctx.author.display_name in str:
                del self.res_b4[i]
        for i, str in enumerate(self.res_b5):
            if ctx.author.display_name in str:
                del self.res_b5[i]
